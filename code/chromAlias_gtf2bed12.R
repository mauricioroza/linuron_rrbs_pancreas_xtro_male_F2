...
# Change chromosome names to assembly format
...
Xtro10_ensemble_gtf2bed_assembly_Chr = data.frame(Xtro10_ensemble_gtf2bed) %>%
  left_join(
    dplyr::select(chr_alias, ensembl, assembly),
    by = c("V1" = "ensembl")
  ) %>%
  mutate(V1 = coalesce(assembly, V1)) %>%
  dplyr::select(-assembly)

levels(as.factor(Xtro10_ensemble_gtf2bed_assembly_Chr$V1))

#Save .bed file with changed chromosome name format
write.table(Xtro10_ensemble_gtf2bed_assembly_Chr, file = "./annotations/Xtro10_ensemble_gtf2bed_assembly_chr.bed", row.names = FALSE, col.names = FALSE, sep = "\t", quote = FALSE)

Xtro10_ensemble_gtf2bed_assembly_Chr <- read.table("./annotations/Xtro10_ensemble_gtf2bed_assembly_chr.bed", header = FALSE)
